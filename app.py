...
app.config['MYSQL_PASSWORD'] = "ammulu@jesus"
...
def notes():
    if session.get('user'):
        ...
        #we give dara as source for template
...
def filehome():
    if session.get('user'):
        ...
        if request.method=='POST':
            ...
            cursor.execute('select * from files files where rollno=%s',[session.get('user')])
            filesdata=cursor.fetchall()
            cursor.close()
...
def deletefile(fid):
    if session.get('user'):
        cursor=mysql.connection.cursor()
        cursor.execution('delete from files where fid=%s',[fid])
        mysql.connection.commit()
        cursor.close()
